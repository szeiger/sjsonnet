...
class Evaluator(resolver: CachedResolver,
  ...
  def visitExpr(expr: Expr)
    ...
    expr match {
      ...
      case ResolvedImport(pos, path, expr, fs) => visitResolvedImport(pos, path, expr, fs)
  ...
  def visitResolvedImport(pos: Position, path: Path, expr: Expr, fs: FileScope)(implicit scope: ValScope) =
    cachedImports.getOrElseUpdate(
      path,
      try visitExpr(expr)(Std.scope(fs.nameIndices.size)) catch Error.tryCatchWrap(pos)
    )
