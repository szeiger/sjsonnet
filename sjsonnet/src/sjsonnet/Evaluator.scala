...
class Evaluator(parseCache: collection.mutable.HashMap[(Path, String), fastparse.Parsed[(Expr, FileScope)]],
  ...
  private def visitApplyBuiltin1(pos: Position, func: Val.Builtin1, a1: Expr)
                                (implicit scope: ValScope) =
    try func.apply1(visitExpr(a1), pos) catch Error.tryCatchWrap(pos)

  private def visitApplyBuiltin2(pos: Position, func: Val.Builtin2, a1: Expr, a2: Expr)
                                (implicit scope: ValScope) =
    try func.apply2(visitExpr(a1), visitExpr(a2), pos) catch Error.tryCatchWrap(pos)
  ...
  private def visitApplyBuiltin(pos: Position, func: Val.Builtin, argExprs: Array[Expr])
    ...
    while (idx < argExprs.length) {
      val boundIdx = idx
      arr(idx) = new Val.Strict(visitExpr(argExprs(boundIdx)))
      idx += 1
    }
    try func.apply(null, arr, pos) catch Error.tryCatchWrap(pos)
  }
