      ...
      res = res0 match{
        case f: Val.Func =>
          ...
          new Val.Func(f.pos, f.defSiteValScope, Params(f.params.names, defaults2)) {
            def evalRhs(vs: ValScope, es: Evaluator, fs: FileScope, pos: Position) = f.evalRhs(vs, es, fs, pos)
            override def evalDefault(expr: Expr, vs: ValScope, es: Evaluator) = f.evalDefault(expr, vs, es)
          }
        case x => x
      }
    } yield res
