  ...
  val functions: Seq[(String, Val.Func)] = Seq(
    ...
    builtin("prune", "x"){ (pos, ev, s: Val) =>
      ...
      def rec(x: Val): Val = x match{
        ...
        case a: Val.Arr =>
          new Val.Arr(pos, a.asStrictArray.iterator.map(rec).filter(filter).toArray)
        case _ => x
      }
      rec(s)
    },
      ...
      case vs: Val.Arr =>
        new Val.Arr(
          ...
          if (vs.forall(_.isInstanceOf[Val.Str])){
            vs.asStrictArray.map(_.cast[Val.Str]).sortBy(_.value).asInstanceOf[Array[Val.Lazy]]
          }else if (vs.forall(_.isInstanceOf[Val.Num])) {
            vs.asStrictArray.map(_.cast[Val.Num]).sortBy(_.value).asInstanceOf[Array[Val.Lazy]]
            ...
            if (keyF.isInstanceOf[Val.False]) {
              ...
              val keyFFunc = keyF.asInstanceOf[Val.Func]
              val keys = objs.map((v) => keyFFunc(null, Array(v), pos.noOffset)(ev))

              if (keys.forall(_.isInstanceOf[Val.Str])){
                objs.sortBy((v) => keyFFunc(null, Array(v), pos.noOffset)(ev).cast[Val.Str].value).map(x => (() => x): Val.Lazy)
              } else if (keys.forall(_.isInstanceOf[Val.Num])) {
                objs.sortBy((v) => keyFFunc(null, Array(v), pos.noOffset)(ev).cast[Val.Num].value).map(x => (() => x): Val.Lazy)
              } else {
