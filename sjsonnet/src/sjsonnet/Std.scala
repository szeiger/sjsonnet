...
object Std {
  ...
  val functions: Seq[(String, Val.Func)] = Seq(
    ...
    builtin("objectFields", "o"){ (pos, ev, fs, v1: Val.Obj) =>
      ...
      Val.Arr(pos, keys.map(k => (() => Val.Str(pos, k)): Lazy))
    },
    ...
    builtin("range", "from", "to"){ (pos, ev, fs, from: Int, to: Int) =>
      Val.Arr(
        pos,
        (from to to).map(i => (() => Val.Num(pos, i)): Lazy).toArray
      )
    },
    ...
    builtin("makeArray", "sz", "func"){ (pos, ev, fs, sz: Int, func: Applyer) =>
      Val.Arr(
        pos,
        {
          ...
          while(i < sz) {
            val forcedI = i
            a(i) = (() => func.apply(Val.Num(pos, forcedI))): Lazy
            i += 1
          }
          a
        }
      ...
      new Val.Obj(
        ...
        allKeys.map{ k =>
          k -> (Val.Obj.Member(false, Visibility.Normal, (self: Val.Obj, sup: Val.Obj, _, _) =>
            func.apply(
              Val.Str(pos, k),
              (() => obj.value(k, fs.noOffsetPos)(ev)): Lazy
            )
          ))
        },
        null,
    ...
    builtin("mapWithIndex", "func", "arr"){ (pos, ev, fs, func: Applyer, arr: Val.Arr) =>
      Val.Arr(
        ...
        arr.value.zipWithIndex.map{ case (x, i) =>
          (() => func.apply(Val.Num(pos, i), x)): Lazy
        }
      )
    },
        ...
        case a: Val.Arr =>
          val arrResults = a.value.flatMap {
            v => {
              ...
              fres match {
                case va: Val.Arr => va.value.asInstanceOf[Array[Eval]]
    ...
    builtin("filterMap", "filter_func", "map_func", "arr"){ (pos, ev, fs, filter_func: Applyer, map_func: Applyer, arr: Val.Arr) =>
      Val.Arr(
        ...
        arr.value.flatMap { i =>
          ...
          else Some[Lazy](() => map_func.apply(x))
        }
      )
    },
    ...
    builtin("findSubstr", "pat", "str") { (pos, ev, fs, pat: String, str: String) =>
      if (pat.length == 0) Val.Arr(pos, emptyLazyArray)
        ...
        Val.Arr(pos, indices.map(x => (() => Val.Num(pos, x)): Lazy).toArray)
      }
    },
    ...
    builtin("join", "sep", "arr"){ (pos, ev, fs, sep: Val, arr: Val.Arr) =>
      val res: Val = sep match{
        ...
        case Val.Arr(_, sep) =>
          val out = new mutable.ArrayBuffer[Eval]
          for(x <- arr.value){
            x.force match{
    ...
    builtin("base64DecodeBytes", "s"){ (pos, ev, fs, s: String) =>
      Val.Arr(pos, Base64.getDecoder().decode(s).map(i => (() => Val.Num(pos, i)): Lazy))
    },
    ...
    builtin("encodeUTF8", "s"){ (pos, ev, fs, s: String) =>
      Val.Arr(pos, s.getBytes(UTF_8).map(i => (() => Val.Num(pos, i & 0xff)): Lazy))
    },
    ...
    builtin("split", "str", "c"){ (pos, ev, fs, str: String, c: String) =>
      Val.Arr(pos, str.split(java.util.regex.Pattern.quote(c), -1).map(s => (() => Val.Str(pos, s)): Lazy))
    },
    builtin("splitLimit", "str", "c", "maxSplits"){ (pos, ev, fs, str: String, c: String, maxSplits: Int) =>
      Val.Arr(pos, str.split(java.util.regex.Pattern.quote(c), maxSplits + 1).map(s => (() => Val.Str(pos, s)): Lazy))
    },
      ...
      def recursiveTransform(js: ujson.Value): Val = {
        js match {
          ...
          case ujson.Arr(values) =>
            val transformedValue: Array[_ <: Eval] = values.map(v => (() => recursiveTransform(v)): Lazy).toArray
    ...
    builtin("prune", "x"){ (pos, ev, fs, s: Val) =>
      ...
      def rec(x: Val): Val = x match{
        ...
        case a: Val.Arr =>
          Val.Arr(pos, a.value.map(x => rec(x.force)).filter(filter).map(x => (() => x): Lazy))
        case _ => x
      }
      rec(s)
    },
  ...
  def scope(size: Int) = {
    new ValScope(
      null, null, null, Array[Eval](Std).padTo(size, null)
    )
  }
  ...
  def sortArr(pos: Position, ev: EvalScope, arr: Val, keyF: Val) = {
    arr match{
      case Val.Arr(_, vs) =>
        Val.Arr(
          ...
          if (vs.forall(_.force.isInstanceOf[Val.Str])){
            vs.map(_.force.cast[Val.Str]).sortBy(_.value).map(x => (() => x): Lazy)
          }else if (vs.forall(_.force.isInstanceOf[Val.Num])) {
            vs.map(_.force.cast[Val.Num]).sortBy(_.value).map(x => (() => x): Lazy)
            ...
            if (keyF.isInstanceOf[Val.False]) {
              ...
              val keyFFunc = keyF.asInstanceOf[Val.Func]
              val keyFApplyer = Applyer(keyFFunc, ev, null)
              val keys = objs.map((v) => keyFApplyer((() => v): Lazy))

              if (keys.forall(_.isInstanceOf[Val.Str])){
                objs.sortBy((v) => keyFApplyer((() => v): Lazy).cast[Val.Str].value).map(x => (() => x): Lazy)
              } else if (keys.forall(_.isInstanceOf[Val.Num])) {
                objs.sortBy((v) => keyFApplyer((() => v): Lazy).cast[Val.Num].value).map(x => (() => x): Lazy)
              } else {
      ...
      case Val.Str(pos, s) => Val.Arr(pos, s.sorted.map(c => (() => Val.Str(pos, c.toString)): Lazy).toArray)
  ...
  def stringChars(pos: Position, str: String): Val.Arr = {
    ...
    Val.Arr(pos, output.map(s => (() => Val.Str(pos, s.toString())): Lazy))
  }
  ...
  def getObjValuesFromKeys(pos: Position, ev: EvalScope, fs: FileScope, v1: Val.Obj, keys: Array[String]): Val.Arr = {
    Val.Arr(pos, keys.map { k =>
      (() => v1.value(k, fs.noOffsetPos)(ev)): Lazy
    })
  }
}
